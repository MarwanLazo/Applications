...
public class TempTableRESTController {
	...
	public ResponseEntity<List<TempTableDTO>> getAllTempTable() {
		List<TempTableDTO> temps = temp.findAllTempTable();
		if (temps.isEmpty()) {
	...
	public ResponseEntity<Void> createUser(@RequestBody TempTableDTO tempTableDTO, UriComponentsBuilder ucBuilder) {
		System.out.println("Creating User " + tempTableDTO.getTempName());
		temp.createTempTable(tempTableDTO);
		HttpHeaders headers = new HttpHeaders();
		headers.setLocation(ucBuilder.path("/employee/{tempName}").buildAndExpand(tempTableDTO.getTempName()).toUri());
